...
// vi toi da chi 100 dau sach nen su dung tim kiem tuyen tinh
...
//dung selection sort vi dau sach it ... khong dung quicksort
void selectionSort_DS(LIST_DAUSACH& lDS)
{
	...
	for (i = 0; i <= lDS.n - 1; i++)
	{
		...
		// doi cho phan tu nho nhat vua tim thay voi phan tu dau tien
		swap_DS(lDS.ListDS[min_idx], lDS.ListDS[i]);
	}
}

//cac ham dieu khien dau sach
...
void Output_DS(dauSach ds)
{
	...
	cout << ds.tensach;
	gotoxy(xDisplayDS[1] + 0, yDisplay + 3 + locate);// moi sua cho nay
	cout << ds.ISBN;
...
void Output_ListDStheoTT(LIST_DAUSACH lDS)
{
	...
	clrscr();
	normalBGColor();
	selectionSort_DS(lDS);//ham sap xep ten sach giam dan
