...
void AddHeadList_MT(LIST_MT& l, MUON_TRA data)
{
	...
	if (ListMTIsEmpty(l)) //Neu danh sach rong
	{
		l.pHead = p;
		l.pTail = p;
	}
...
void AddTailList_MT(LIST_MT& l, MUON_TRA data)
{
	...
	{
		l.pTail->pNext = p; //ket noi voi danh sach
		p->pPre = l.pTail; //P tro ve Node truoc
		l.pTail = p; //luu lai vi tri cuoi
	}
	++l.n;
}
...
void DelHeadList_MT(LIST_MT& l, MUON_TRA data) {
	if (!ListMTIsEmpty(l))
	{
		data = l.pHead->data; //lay gia tri ra neu can dung
		l.pHead = l.pHead->pNext; //Cho L tro den Node thu 2 trong danh sach
	}
}
