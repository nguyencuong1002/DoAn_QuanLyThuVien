...
void Introduce()
{
	...
	ifstream inFile("ManHinhDau.txt", ios::in);
	...
	{
		cout << "Lien ket voi File ManHinhDau.txt khong thanh cong! " << "\n";
	}

	SetColor(WHITE);
	gotoxy(xTen, yTen);
	cout << "HOTEN : NGUYEN THE CUONG";
	gotoxy(xTen, yTen + 1);
	cout << "MSSV  : N17DCAT009";
	gotoxy(xTen, yTen + 2);
	cout << "LOP   : D17CQAT01-N";
...
void ManHinhChinh()
{
	...
	ifstream inFile("QLTV.txt", ios::in);
	...
	{
		cout << "Lien ket voi File QLTV.txt khong thanh cong! " << "\n";
	}
...
{
	fstream outFile;
	outFile.open("dsDocGia.txt", ios::out);
	if (outFile.is_open())
	{
		// so doc gia..
...
{
	fstream outFile;
	outFile.open("dsDauSach.txt", ios::out);
	if (outFile.is_open())
	{
...
void Load_DS(LIST_DAUSACH& lDS)
{
	fstream inFile;
	dauSach info;
	inFile.open("dsDauSach.txt", ios::in);
	pDAU_SACH pDS;
	DMS dms;
	...
	if (inFile.is_open())
	{
		...
		cout << "KET NOI VOI FILE DAU SACH THAT BAI! ";
	}
...
void Load_DG(TREE_DG& t)
{
	...
	DOC_GIA dg;
	MUON_TRA mt;
	inFile.open("dsDocGia.txt", ios::in);
	...
	if (inFile.is_open())
	{
		...
		cout << "KET NOI VOI FILE DOC GIA THAT BAI! ";
	}
